...
class GetMessagesUseCaseTest {
    ...
    private lateinit var usecase: GetMessages
    ...
    fun shouldReturnFlowOfMessagesUser1User2() = runBlocking {
        ...
        usecase.getAllMessages(user1.id, user2.id).collect {
            it?.let {
                for (i in it) {
                    messageList.add(i)
                }
        ...
        Assert.assertEquals(messagesUser1User2, messageList)
        Assert.assertNotEquals(allMessages, messageList)
    }
